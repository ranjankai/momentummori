...
## Session 01-Sep-2026 — the "Exited" duplication bug lived in TWO builders, `daily_report.build()` confirmed dead and deleted, NSE tick-size rounding built

### The "Exited" same-day duplication fix from 31-Aug didn't actually fix production

The 31-Aug fix (above) excluded today's exits from `exited_review` inside `daily_report.build()`. The very next live daily note (01-Sep) still showed KALYANKJIL/NAM-INDIA duplicated between "Exits today" and "Exited". Root cause: `daily_report.build()` and `cycle_state.py`'s `to_report()` are two separate, independent functions that each assemble a `daily_report.Report` and feed it to the same shared `render()` -- and `cmd_daily` has called `cycle_state.build()`/`to_report()` since the incremental rewrite, never `daily_report.build()`. The 31-Aug fix landed in the dead copy. `cycle_state.py` had its own, unfixed copy of the same loop (its own comment, dated 17-Aug, already documented this EXACT same mistake happening once before with a different section, "SELL ORDERS" -- a fix landing in `daily_report.build()`, discovered dead, the live bug found and fixed separately in `cycle_state.py`). Fixed the live copy with the identical same-day filter, verified against the actual `cycle_state.json` (KALYANKJIL/NAM-INDIA correctly dropped, BANDHANBNK/NATIONALUM/INOXWIND from the prior day correctly retained).

### Why the duplication kept recurring -- audited, not guessed

Asked directly why "we thought we cleaned everything" and there were still two copies. Full audit (not from memory): `daily_report.build()` has zero real callers anywhere -- not `run_strategy.py`, no other entry point, no test (its own docstring claimed a `tests/test_daily_report.py` parity test existed; that file does not exist in the repo). The 25-Aug "book.json single source of truth" session was scoped only to the book.json data file; it briefly re-confirmed `build()` was alive because `build_entry_sheet` still called it then, but a separate 26-Aug fix decoupled `build_entry_sheet` from `build()` entirely, silently re-orphaning it, and nobody circled back. Two independent Report-assembly implementations, zero test coverage keeping them in sync -- that's the structural cause, not carelessness on any one fix.

**Deleted `daily_report.build()`** (185 lines) after verifying `empty_slots`' two formulas genuinely differ by design (build() counted identified-but-never-bought mid-cycle candidates as filling a slot; `cycle_state.to_report()` counts actual exits -- correct given `V4_REDEPLOY_ENABLED = False` in production, verified against real `cycle_state.json`: 5 HOLD + 5 EXITED = 10, zero empty). `governing_expiry()` (used by both `build_entry_sheet` and `cycle_state.py`) was kept. 59/59 tests still pass.

**Follow-up dead-code audit** (requested explicitly) found three more orphans, all confirmed zero-callers-anywhere before removal: `strategy.Position.pct_to_target` (sibling of `pnl_pct`/`target_placeable`, itself never used), `cycle_state.summarise()` (superseded by `to_report()`, never called), `book.get_archived()` (built specifically as a `daily_report.build()` fallback, orphaned the moment that function was deleted). Also found, and removed on request: `llm_judgment.rs_rank()`/`choose_candidate()` and the candidate prompt/schema, plus `config.LLM_CANDIDATE_ENABLED`/`RS_WEIGHTS`/`LLM_DEPLOY_MIN_PCT` -- the entire mid-month LLM-candidate-replacement chain, orphaned the same way (only ever called from the now-deleted `build()`; already inert in production since `V4_REDEPLOY_ENABLED = False`). `README.md` flagged as stale (still documents the retired wsgiref dashboard) but not touched -- doc, not code, left for a separate pass.

### NSE tick-size rounding built and wired in

Reopened from the 27-Aug "leave it to the investor" decision after a concrete counter-example: POWERINDIA trades above Rs 20,000, where the NSE tick is Rs 5 (price-band regime, last revised 15-Apr-2025), and its live SL of 31,706.25 was never a placeable price at all (nearest valid: 31,705 or 31,710) -- a materially different situation from the sub-Rs-1000 names, where the gap is a few paise. Built `daily_report._round_to_tick()` (band table: <250 -> 0.01, <=1000 -> 0.05, <=5000 -> 0.10, <=10000 -> 0.50, <=20000 -> 1.00, else 5.00) and wired it into every place a price becomes a real order: `_solve_shares_to_slot` (rounds before computing deviation, so the tie-break judges the actually-placeable price) and `_vol_requote` round internally now; all 14 `sl_price`/`exit_price` assignments in `entry_tracking.py` (including `reconcile()`'s fresh-recompute paths and `apply_reconcile_fixes()`) go through it; and `cycle_state.py`'s PENDING->HOLD stop/target derivation (line ~282) -- the actual live source of what "CONTINUE TO HOLD" shows every evening, confirmed via `cycle_state.json` as POWERINDIA's real SL source, not `entry_tracking.py`. Verified against real numbers throughout (POWERINDIA: 31706.25 -> 31705.00; a synthetic 3-stock sizing run confirmed every resulting limit price lands on a valid tick with deviation still well inside `ENTRY_MAX_WEIGHT_DEV_PCT`). Today's already-persisted `cycle_state.json` had the stale POWERINDIA stop corrected too (backed up first, derived via the same `_round_to_tick` function, not typed) -- explicit go-ahead given before writing to live data, per this file's own Working Rule above.
